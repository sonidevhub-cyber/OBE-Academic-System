...
def calculate_course_ga_score(course_session: CourseSession, ga: GA):
    ...
    enrolled_students = Student.objects.filter(user__batch=course_session.batch).count()
...
def calculate_ga_attainment_semester_cohort(batch: Batch, semester: Semester, ga: GA):
    ...
    # Calculate weighted average: sum(score * enrolled_students) / sum(enrolled_students)
    total_weighted_score = Decimal('0')
    total_enrolled = 0

    for cs in course_scores:
        # Try to get enrolled_students, fallback to counting from StudentCLOScore
        try:
            enrolled = cs.enrolled_students
        except Exception:
            enrolled = StudentCLOScore.objects.filter(course_session=cs.course_session).values('student').distinct().count()
        total_weighted_score += cs.score * enrolled
        total_enrolled += enrolled

    if total_enrolled == 0:
        return Decimal('0')

    return round(total_weighted_score / total_enrolled, 2)


def calculate_ga_attainment_cumulative_cohort(batch: Batch, ga: GA):
    """
    Calculate cumulative cohort GA attainment: all semesters
    """
    course_sessions = CourseSession.objects.filter(
        batch=batch,
        is_active=True,
