...
def branch_version_if_needed(version, batch, user):
    ...
    with transaction.atomic():
        ...
        existing_versions = CurriculumVersion.objects.filter(
        ...
        for version_no in existing_versions:
            try:
                minor = int(version_no.split('.')[1])
                minor_numbers.append(minor)
            except (IndexError, ValueError):
                continue

        next_minor = max(minor_numbers, default=0) + 1

        new_version_no = f"{major}.{next_minor}"
        ...
        # ----------------------------------------------------
        # IMPORTANT:
        # Only the requested batch moves to the new version.
        # Other batches remain on the source version.
        # ----------------------------------------------------
        batch.curriculum_version = new_version
        batch.save(
            update_fields=['curriculum_version']
        )

        # ----------------------------------------------------
        # Copy courses
        # ----------------------------------------------------
        for vc in version.version_courses.filter(is_active=True):
        ...
        # ----------------------------------------------------
        clone_obe_data(version, new_version)

        # ----------------------------------------------------
        # Copy teacher allocations belonging to this batch
        # ----------------------------------------------------
...
def clone_obe_data(source_version, new_version):
    """
...
def suggest_curriculum_for_new_batch(batch):
    ...
    Both can use the same finalized V1.0.

    A clone is created only when the new batch actually needs
...
def activate_curriculum_version(version, activated_by):
    ...
    Progressive:
        Semester-wise curriculum can be prepared gradually.

    Complete:
        Curriculum is expected to be defined completely
        before finalization.
    ...
    IMPORTANT:
        ...
        A previous version is archived only if the SAME
    ...
    with transaction.atomic():
        ...
        if version.curriculum_mode == 'complete':
            ...
            total_semesters = version.program.total_semesters
            ...
            if missing_semesters:
                raise ValidationError(
                    ...
                    f"for all semesters. Missing semesters: "
        ...
        # ----------------------------------------------------
        # Archive an older finalized version ONLY if the same
        # active batch is being moved from that version.
        # ----------------------------------------------------
        previous_versions = CurriculumVersion.objects.filter(
            program=version.program,
            status='finalized',
            is_active=True,
        ...
        for previous_version in previous_versions:
            ...
            if active_batch_ids.intersection(previous_batch_ids):
...
def validate_semester_editable(version, semester_no, batch=None):
    ...
    if version.curriculum_mode == 'progressive':
        ...
        if batch.curriculum_version_id != version.id:
            raise ValidationError(
                "This batch is not assigned to this curriculum version."
            )

        current_semester = batch.current_semester

        # Previous semesters are locked
        if semester_no < current_semester:
            raise ValidationError(
                f"Semester {semester_no} is locked for batch "
                f"{batch.name}. Current semester is "
                f"{current_semester}."
            )

        # Only current semester can be edited
        if semester_no > current_semester:
            raise ValidationError(
                f"Semester {semester_no} is not unlocked yet. "
                f"Current semester is {current_semester}."
            )
...
def clone_allocations_for_version(
    ...
    This prevents Batch 2025's allocations from being copied
    into Batch 2026 accidentally.
    """
