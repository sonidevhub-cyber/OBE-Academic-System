  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        ...
        <p className="text-gray-500 animate-pulse">Loading pending transfers...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-8">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-500 mb-2">
        ...
        <span className="text-gray-400 font-medium">Pending Transfers</span>
      </nav>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b pb-6 border-gray-100">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight flex items-center gap-3">
            <MoveHorizontal className="w-8 h-8 text-orange-600" />
            <span>Pending Transfers</span>
          </h1>
          <p className="text-gray-500 mt-1">Students waiting for an eligible batch to repeat their semester.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex items-center gap-4">
          <div className="p-3 bg-orange-50 rounded-2xl"><Users className="w-6 h-6 text-orange-600" /></div>
          <div>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex items-center gap-4">
          <div className="p-3 bg-amber-50 rounded-2xl"><AlertCircle className="w-6 h-6 text-amber-600" /></div>
          <div>
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-gray-50">
            {filteredStudents.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-6 py-20 text-center text-gray-400 italic">No students waiting for transfer.</td>
              </tr>
            ) : (
                  ...
                  <td className="px-6 py-5 text-right">
                    {s.has_eligible_batch ? (
                      <button 
                        ...
                        className="flex items-center gap-1.5 px-4 py-2 bg-orange-50 text-orange-600 rounded-xl hover:bg-orange-600 hover:text-white transition-all text-xs font-bold ml-auto border border-orange-100"
                      >
